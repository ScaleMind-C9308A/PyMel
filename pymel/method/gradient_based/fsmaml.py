...
def fsmaml(
    ds_cfg: DSConfig, tr_cfg: TrainConfig, 
    model: nn.Module = None, gpus: List[int] = ...,
    meta_opt: str = None, 
    meta_lr: float = 0.001,
    meta_wd: float = 1e-4,
    ...
    args.model = model
    args.gpus = gpus
    args.meta_opt = meta_opt
    args.sp_opt = sp_opt
    args.meta_lr = meta_lr
    args.meta_wd = meta_wd
    args.sp_lr = sp_lr
    args.sp_wd = sp_wd
    args.crit = criterion        
    args.outer_epoch = outer_epoch
    args.inner_epoch = inner_epoch
    
    mp.spawn(main_worker, (args,), nprocs = args.world_size)
...
def main_worker(gpu, args):
    ...
    assert batch_size % args.world_size == 0
    assert args.ds_cfg.get_k_shot() % args.world_size == 0
    assert args.ds_cfg.get_k_query() % args.world_size == 0
    
    train_sampler = DistributedSampler(args.ds_cfg.train_ds)
    test_sampler = DistributedSampler(args.ds_cfg.test_ds)
